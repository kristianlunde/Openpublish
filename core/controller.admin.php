...
class controller_admin extends opControllerBase implements SplSubject {
    ...
    public function index() {
        if ($this->authenticate()) {
            ...
            $template = new opHtmlTemplate('<a href="/admin/opUsers/userEdit/'.$this->activeUser->getId().'" title="'.$this->activeUser->getFullName().'"><span><img src="/themes/opAdmin/images/icons/user.png" class="btnIcon" alt="'.$this->activeUser->getFullName().'" /> '.$this->activeUser->getFullName().'</span></a>');
            $templateCollection->addTemplate($template);
            $templateManager->addCollection($templateCollection);

            # Logout
            $templateCollection = new opTemplateCollection('logout');
            $template = new opHtmlTemplate('<a href="/admin/logout" title="'.opTranslation::getTranslation('_logout').'"><span><img src="/themes/opAdmin/images/icons/lock-unlock.png" class="btnIcon" alt="'.opTranslation::getTranslation('_logout').'" /> '.opTranslation::getTranslation('_logout').'</span></a>');
            $templateCollection->addTemplate($template);
            $templateManager->addCollection($templateCollection);
    ...
    private function buildNav() {
        ...
        foreach ($leftArr as $k => $v) {
            ...
            $html .= '<li><a'.$activeCSS.' href="/admin/'.$v.'"><span>'.$k.'</span></a></li>';
            $i++;
        }
        $html .= '</ul>';
    ...
    private function getTranslationList() {
        ...
        foreach ($translations as $translation) {
            $list .= '<li><a href="/admin/opTranslation/setTranslation/'.$translation['code'].'"><img src="/core/plugins/opTranslation/flags/'.$translation['code'].'.png" class="languageFlag" alt="'.$translation['name_na'].'" /> '.$translation['name_na'].'</a></li>';
        }
        $list .= '<li><a href="/admin/opTranslation"><img src="/core/plugins/opTranslation/icons/globe.png" class="translateFlag" alt="'.opTranslation::getTranslation('_translations').'" /> '.opTranslation::getTranslation('_translations').'</a></li>';
        $list .= '</ul>';
